...
RUN mkdir /tmp/bin && \
    curl https://github.com/jqlang/jq/releases/download/jq-1.6/jq-linux64 > /tmp/bin/jq && \
    ...
    tar xvf /tmp/oc/openshift-client-linux.tar.gz&& \
    cd /tmp/oc/ && \
    mv oc /tmp/bin/oc

COPY . /opt/app-root/src

RUN export PATH=$PATH:/tmp/bin/ && python3 -m pip install analytics
